...
/**
 * Get the component resolution for preview
 * Returns which components should use user's implementation vs bundled
 */
export async function resolveComponents(
  ...
  Map<
    string,
    { useUser: boolean; path: string | null }
  >
> {
  ...
  for (const name of componentNames) {
    const userPath = await hasUserComponent(projectRoot, name);
    resolution.set(name, {
      useUser: userPath !== null,
      path: userPath
    });
  }
...
/**
 * Get list of Code Hike components that user has NOT implemented
 */
...
/**
 * Get list of Code Hike components that user HAS implemented
 */
export async function getExistingComponents(
  projectRoot: string,
  usedComponents: string[]
): Promise<string[]> {
